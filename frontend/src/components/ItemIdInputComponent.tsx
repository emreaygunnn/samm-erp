    ...
    return (
      <div className="card" style={{ padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 6 }}>
              {t('productUpdate.productIds')}
            </label>
            <textarea
              className="form-input"
              rows={4}
              style={{ resize: "vertical", fontFamily: "monospace", fontSize: 13 }}
              placeholder={t('productUpdate.productIdsPlaceholder')}
              value={rawText}
          ...
          <div style={{ flexShrink: 0, paddingTop: 22 }}>
            ...
            <button
              ...
              title={t('productUpdate.uploadExcelHint')}
            >
              <Upload size={14} /> {t('productUpdate.uploadExcel')}
            </button>
          </div>
        </div>
        ...
        <div style={{ marginTop: 8, fontSize: 12, color: "var(--text-muted)" }}>
          {items.length} {t('productUpdate.idEntered')}
          {items.filter((item) => item.value !== "").length > 0 && (
            <span> · {items.filter((item) => item.value !== "").length} {t('productUpdate.valueMatched')}</span>
          )}
        </div>
      </div>
    );
