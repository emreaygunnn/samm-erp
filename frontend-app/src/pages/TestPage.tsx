...
export default function UrunGuncellePage() {
  ...
  const handleOperasyonChange = (op: OperationType) => {
    setOperasyon(op);
    // Operasyon değişince tüm item'ların değerlerini sıfırla
    setItems((prev) => prev.map((item) => ({ ...item, value: '' })));
  };

  // ── Tüm item'ların değeri dolu mu? ──
  ...
  const handleGuncelle = async () => {
    ...
    if (!tumDegerliMi) return; // tüm satırların değeri dolu olmalı
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* 3 — Değer Tablosu (operasyon seçili ve en az 1 item varsa göster) */}
